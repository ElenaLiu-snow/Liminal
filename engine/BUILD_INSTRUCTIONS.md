...
- 先分离核心体验、明确表达的情感/现实重量、当前解释框架、正在考虑的决定与关键未知项
- 围绕一个中心心理轴线形成因果或意义主线，不把后台过程映射直接渲染成答案
- 如问题发生变化，区分开始时的心理焦点与揭晓时的表达
- 解释牌如何移动观察视角与判断标准，并至少保留两个尚未排除的竞争性解释
...
后台分析保留盲态发现、问题结构、情境化牌义、继承审计、因果过程综合、视角位移、关键未知项和方向回答等独立字段；用户界面不把这些字段机械拆开。最终只显示：

1. 一篇连续的综合解读：从冻结的心理过程出发，保留问题的重要重量，说明解释框架如何形成决定，并呈现牌带来的不同观察坐标和有边界回答
2. 一个指向关键未知项、但不预设解决方法的带走问题
...
- 输出：与问题相关的牌面结构、正逆位机制、至少两个开放张力轴线和能力边界
- 不输出方向答案、行动建议或反思问题，避免在因果综合之前预设沟通、坚持或离开
...
截至 2026-09-14，真实模型人工试读已通过 Pass 1；Pass 2 先后暴露的“形式继承但未充分继承心理过程”和“逐项对应造成叙事碎片化”已由因果综合版修复并通过回归验收：

- Phase 1：v2 alpha schema、过程型心理模式、证据引用和认识边界运行时校验已实现
- Phase 2：78 张牌可通过隔离接口生成 provisional canonical reading；situated 请求/输出契约已实现。另已建立 78 张牌的版本化影子知识库、逐牌 Waite 来源链接、78 份描述/正位及 77 份原书逆位候选文本、图像来源候选和审核状态；逐牌人工审校仍属生产完成条件
- Phase 3：事实/转换、过程模式、补偿、写作四阶段 prompt 已隔离；请求构建、结构校验、组装、SHA-256 冻结及防篡改检查已实现
- Phase 4：问题变化状态、situated 传统层、现实证据、有边界方向回答和实践转译已实现；已验收的 Pass 2 基线把逐步映射保留为后台审计，只需至少两个有证据且顺序一致的步骤即可确认过程复现，并采用“现实问题结构—因果过程综合—牌面视角位移—竞争性解释—判断标准—关键未知项”的叙事链；用户侧仅用一句自然来源桥句标明该整体过程来自先前解牌
- 并行基础：分析方法已从 Prompt 中抽离为影子规范，并完成荣格理论、主观验证风险、项目测验争议、定性研究规范与 AI 健康伦理的来源登记、证据映射和争议边界；当前七份人工批准的基线或修订候选 Prompt 已用 SHA-256 锁定。知识与方法层在显式激活前均不能改变正式模型输入
- 模型接入：DeepSeek OpenAI-compatible HTTP 适配器、JSON 模式、运行时结构契约、四阶段 Pass 1 / 三阶段 Pass 2 编排、逐阶段校验与安全回执已实现；`deepseek-v4-pro` 合成全链路已通过
- 当前停止点：Phase 4 已通过真人回归验收；下一步进入 Phase 5 产品展示层，并在 Demo 联调时用新牌和新问题补做完整双阶段确认
